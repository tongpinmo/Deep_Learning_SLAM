...
class SfMLearner(object):
    ...
    def build_train_graph(self):
        ...
        with tf.name_scope("depth_prediction"):
            pred_disp, depth_net_endpoints = disp_net(tgt_image,
                                                      is_training=True)  # true 表示正在训练，false表示正在测试
            pred_depth = [1. / d for d in pred_disp]  # FIXME：1./d 是干啥的？
            ...
            for s in range(opt.num_scales):
                ...
                curr_tgt_image = tf.image.resize_area(tgt_image,
                                                      [int(opt.img_height / (2 ** s)),
                                                       int(opt.img_width / (2 ** s))])  # FIXME：为什么要除以2**s,是每张图像相应缩放么?
                ...
                if opt.smooth_weight > 0:
                    smooth_loss += opt.smooth_weight / (2 ** s) * \
                                   self.compute_smooth_loss(pred_disp[s])  # FIXME：为什么要除以2**s
                ...
                for i in range(opt.num_source):
                    ...
                    if opt.explain_reg_weight > 0:
                        pixel_loss += tf.reduce_mean(curr_proj_error * \
                                                     tf.expand_dims(curr_exp[:, :, :, 1], -1))  # reduce_mean求均值
                    else:
                        pixel_loss += tf.reduce_mean(curr_proj_error)
    ...
    def compute_exp_reg_loss(self, pred, ref):
        l = tf.nn.softmax_cross_entropy_with_logits(
            ...
            logits=tf.reshape(pred, [-1, 2]))  # 进行sotfmax运算
        return tf.reduce_mean(l)  # 对向量求均值，计算loss

    # FIXME：此处计算smooth_loss是什么样的
    ...
    def collect_summaries(self):
        ...
        for s in range(opt.num_scales):
            ...
            tf.summary.image('scale%d_disparity_image' % s, 1. / self.pred_depth[s])  # 此处的1./是什么意思
    ...
    def build_depth_test_graph(self):
        ...
        with tf.name_scope("depth_prediction"):
            ...
            # print(pred_disp)
            pred_depth = [1. / disp for disp in pred_disp]  # FIXME:为什么要1./
            # pred_depth = tf.Print(pred_depth, [pred_depth], "pred_depth : ")
        pred_depth = pred_depth[0]  # shape:[1,128,416,1]
