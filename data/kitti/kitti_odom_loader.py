...
class kitti_odom_loader(object):
    ...
    def is_valid_sample(self, frames, tgt_idx):
        N = len(frames)
        tgt_drive, _ = frames[tgt_idx].split(' ')    #tgt_drive存放分割后前面的序列
        half_offset = int((self.seq_length - 1)/2)   #seq_length=5
    ...
    def load_example(self, frames, tgt_idx, load_pose=False):
        ...
        # print(intrinsics)
        intrinsics = self.scale_intrinsics(intrinsics, zoom_x, zoom_y)   #此处的内参矩阵已经经过了变换
        example = {}
