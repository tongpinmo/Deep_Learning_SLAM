...
def disp_net(tgt_image, is_training=True):
    H = tgt_image.get_shape()[1].value   #128
    W = tgt_image.get_shape()[2].value   #416
...
#FIXME 反卷积
            ...
            i7_in  = tf.concat([upcnv7, cnv6b], axis=3)   # FIXME 第三个维度上concat:[batch,in_height,in_width,in_channels],在channels上进行concat
            ...
            disp4_up = tf.image.resize_bilinear(disp4, [np.int(H/4), np.int(W/4)])#FIXME:双线性插值

        # 双线性插值处理图像大小 tf.image.resize_bilinear(images, size, name=None)

        # FIXME：此处的disp_scaling,min_disp是干什么的？
            ...
            disp3_up = tf.image.resize_bilinear(disp3, [np.int(H/2), np.int(W/2)]) #FIXME: size 怎么确定
            ...
            #FIXME：tf.Print 打印tensor内容
            # disp1 = tf.Print(disp1,[disp1],'disp1_out = ')
            
            end_points = utils.convert_collection_to_dict(end_points_collection)
            return [disp1, disp2, disp3, disp4], end_points
        #demo:disp1:shape=[1,128,416,1];disp2:shape=[1,64,208,1];disp3:shape=[1,32,104,1];disp4:shape=[1,16,52,1];
